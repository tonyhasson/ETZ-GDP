...
#TODO check life expetency at 1960 and 2020 calc diff
#TODO
...
def mix_plot(df):
    ...
    for continent in df["Continent"].unique():
        x=list(df[(df["Continent"]==continent) & (df["Year"]==2020)]["Population Total"])
    ...
    for contint in df["Continent"].unique():
        x=[]
        y=[]
        for year in range(min(df[df["Continent"] == contint]["Year"]), max(df[df["Continent"] == contint]["Year"])):
            x.append((df[(df["Continent"] == contint) & (df["Year"] == year)]["GDP Total"].median()))
            y.append(year)
